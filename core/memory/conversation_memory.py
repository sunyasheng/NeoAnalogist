...
from core.events.observation import (AgentThinkObservation,
                                     ...
                                     CmdOutputObservation, FileEditObservation,
...
class ConversationMemory:
    ...
    def __init__(self, enable_som_visual_browsing: bool, prompt_manager: PromptManager):
        self.enable_som_visual_browsing = enable_som_visual_browsing
    ...
    def process_events(
        ...
        # 使用 condensation view 逻辑裁剪和插入 summary
        events = View.from_events(condensed_history).events

        # # log visual browsing status
        # logger.debug(f'Visual browsing: {self.enable_som_visual_browsing}')
    ...
    def process_initial_messages(
        ...
        system_prompt_name: str = "scientist_system_prompt_w_produce.j2",
        suffix_prompt: str = "",
    ) -> list[Message]:
        ...
        return [
            Message(
                ...
                content=[
                    TextContent(
                        text=self.prompt_manager.get_system_prompt(system_prompt_name)
                        + suffix_prompt,
    ...
    def _process_action(
        ...
        Args:
            action: The action to convert. Can be one of:
                ...
                - TaskGraphBuildAction: For building task graphs
    ...
    def _process_observation(
        ...
        - TaskGraphBuildObservation: Formats task graph building results
        ...
        if isinstance(obs, CmdOutputObservation):
            ...
            if obs.tool_call_metadata is None:
                text = tail_lines(obs.content, 20)
                text = truncate_content(
                    f"\nObserved result of IPython cell executed by user:\n{text}",
                    max_message_chars,
                )
            else:
                text = tail_lines(obs.content, 20)
            ...
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, TaskGraphBuildObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoPlanObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoCreateObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoAnalyzerObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoUpdateObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoVerifyObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, PaperReproductionAnalyzerObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoDebugObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoEditObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, PDFQueryObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, RepoRunObservation):
            text = tail_lines(str(obs), 20)
            text = truncate_content(text, max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
            ...
            text = truncate_content(obs.content, max_message_chars)
            message = Message(role='user', content=[TextContent(text=text)])
        elif isinstance(obs, RepoJudgeObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        elif isinstance(obs, PaperRubricObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        ...
        # GoT edit observation
            ...
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        # Qwen API observation
        elif isinstance(obs, QwenAPIObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        ...
        # AnyDoor edit observation
            ...
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        # GroundingSAM observation
        elif isinstance(obs, GroundingSAMObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        # GroundingDINO observation
        elif isinstance(obs, GroundingDINOObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
        # ImageUnderstanding observation
        elif isinstance(obs, ImageUnderstandingObservation):
            text = truncate_content(str(obs), max_message_chars)
            message = Message(role="user", content=[TextContent(text=text)])
