...
from core.utils.types.agent_state import AgentState
...
class State:
    ...
    def restore_from_session(
        ...
        try:
            ...
            logging.warning(f'agent_state.pkl not found for sid={sid}, user_id={user_id}, trying legacy path...')
            if user_id:
                try:
                    filename = get_conversation_agent_state_filename(sid)
                    ...
                    logging.warning(f'agent_state.pkl not found for sid={sid} (legacy path). Returning new State instance, will restore event stream only.')
                    return State()
            else:
                logging.warning(f'agent_state.pkl not found for sid={sid}. Returning new State instance, will restore event stream only.')
                return State()
        except Exception as e:
    ...
    def to_llm_metadata(self, agent_name: str) -> dict:
        return {
            ...
            'trace_version': 'unknown',  # openhands.__version__ if available
            'tags': [
                f'agent:{agent_name}',
                f'web_host:{os.environ.get("WEB_HOST", "unspecified")}',
                f'openhands_version:unknown',  # openhands.__version__ if available
            ],
        }
