        ...
        if [ -d "$paper_dir" ]; then
            ...
            if [ "$is_whitelisted" = true ]; then
                ...
                if [ -n "$WORK_DIR" ]; then
                    ...
                    PYTHONPATH=./ python main.py --config config/paperbench_config_w_condenser_claude.json --task "reproduce a paper for me" --max-steps 300 --task-type paperbench --paper-path "$paper_path" --work-dir "$WORK_DIR"
                else
                    PYTHONPATH=./ python main.py --config config/paperbench_config_w_condenser_claude.json --task "reproduce a paper for me" --max-steps 300 --task-type paperbench --paper-path "$paper_path"
                fi
            else
