...
def build_command_with_planner(
    main_py: Path, 
    work_dir: Path, 
    ...
    """Build command for main_with_planner.py with Memento planner options"""
    cmd = [
        "python",
        str(main_py),
    ...
    # Memento planner specific options
    cmd.extend(["--max-cycles", str(max_cycles)])
    cmd.extend(["--max-steps-per-task", str(max_steps_per_task)])
    cmd.extend(["--timeout-per-task", str(timeout_per_task)])
...
def main():
    ...
    default_main_py = repo_root / "main_with_planner.py"  # Use the new planner main

    parser = argparse.ArgumentParser(description="Batch prepare and run ReasonEdit tasks with Memento Planner")
    ...
    parser.add_argument("--run", action="store_true", help="If set, invoke main_with_planner.py for each task")
    parser.add_argument("--config", type=str, default=str(default_config),
                        help="Config file passed to main_with_planner.py if --run is set")
    parser.add_argument("--task-template", type=str, default=None,
                        help="Optional task text passed to main_with_planner.py --task. Supports {category}, {task_id}, {instruction} placeholders.")
    ...
    # Memento planner specific arguments
    parser.add_argument("--max-cycles", type=int, default=3,
                        help="Maximum planning cycles for Memento planner")
    parser.add_argument("--max-steps-per-task", type=int, default=5,
                        help="Maximum steps per task for Memento planner")
    parser.add_argument("--timeout-per-task", type=int, default=60,
                        help="Timeout per task in seconds for Memento planner")
    parser.add_argument("--memory-path", type=str, default="core/memento/memory/cases.jsonl",
    ...
    parser.add_argument("--debug", action="store_true",
                        help="Enable debug mode for Memento planner")
    ...
    if not args.grouped:
        ...
        for category, cat_dir in categories:
            ...
            for task_id in chosen:
                ...
                # Optional run with Memento planner
                if args.run:
                    ...
                    cmd = build_command_with_planner(
                        main_py, work_dir, resolved_config, task_text, repo_root,
                ...
                total += 1
        print(f"Prepared {total} task workspaces with Memento Planner (per-task mode)")
        ...
        for category, cat_dir in categories:
            ...
            total += len(summary)
        print(f"Prepared {total} tasks with Memento Planner under separate timestamped roots (one per category)")
    else:
        ...
        with index_path.open("w") as f:
            ...
            for category, task_id, dest_dir, instruction in summary:
                f.write(f"{category}\t{task_id}\t{dest_dir}\t{(instruction or '').replace('\t',' ').replace('\n',' ')}\n")
        print(f"Prepared {len(summary)} tasks with Memento Planner under {dest_root}")
